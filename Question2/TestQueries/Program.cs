...
from 
  ...
  patients p
    on r.id=(
      select top 1 id from receptions where id_patients=p.id and StartDateTime=(
        select max(StartDateTime) from receptions where id_patients=p.id order
      )
    )
    inner join
  persons per
